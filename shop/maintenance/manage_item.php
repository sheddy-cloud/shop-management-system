  ...
  $(document).ready(function(){
    ...
    $('#item-form').submit(function(e){
        ...
        if(!validateData($(this))) {
            var errMsg = $('<div>').addClass("alert alert-danger err-msg").text("Invalid inputs are not allowed");
            _this.prepend(errMsg);
            errMsg.show('slow');
            return;
        }
        start_loader();
...
function validateData(form) {
    ...
    for (const pair of formData.entries()) {
        if (/[#?!]/.test(pair[1]) ) {
            // Invalid character found (contains #, ?, or !)
            return false;
        }
    }
    // All characters are valid (no #, ?, or !)
    return true;
}
