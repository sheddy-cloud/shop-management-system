...
<div class="card card-outline card-primary">
    ...
    <div class="card-body">
        ...
        <div class="alert alert-info">
            ...
            <strong>Stock Validation:</strong> The system will automatically check available stock and prevent sales that would result in negative quantities. Select an item to see current availability.
        </div>
    ...
    <div class="card-footer py-1 text-center">
        ...
        <a class="btn btn-flat btn-dark" href="<?php echo base_url.'/shop?page=sale' ?>">Cancel</a>
    </div>
</div>
...
<script>
    ...
    function getCurrentStock(itemId) {
        return new Promise((resolve, reject) => {
            $.ajax({
                url: _base_url_ + 'classes/Master.php?f=get_current_stock',
                method: 'POST',
                data: { item_id: itemId },
                dataType: 'json',
                success: function(response) {
                    if(response.status == 'success') {
                        resolve(response.current_stock);
                    } else {
                        resolve(0);
                    }
                },
    ...
    $(function(){
        ...
        $('#item_id').on('change', function(){
            var itemId = $(this).val();
            if(itemId) {
                getCurrentStock(itemId).then(function(currentStock) {
                    $('#qty').attr('max', currentStock);
                    $('#qty').attr('placeholder', 'Available: ' + currentStock);
                    if(currentStock <= 0) {
                        alert_toast('This item is out of stock!', 'warning');
                        $('#qty').prop('disabled', true);
                        $('#stock-info').html('<span class="text-danger">Out of stock</span>');
                    } else {
                        $('#qty').prop('disabled', false);
                        $('#stock-info').html('<span class="text-success">Available: ' + currentStock + ' units</span>');
                    }
                });
            } else {
                $('#qty').prop('disabled', true);
        ...
        $('#add_to_list').click(function(){
            ...
            getCurrentStock(item).then(function(currentStock) {
                if(parseFloat(qty) > currentStock) {
                    alert_toast('Insufficient stock! Available: ' + currentStock + ', Requested: ' + qty, 'error');
                    return false;
                }
			...
			$.ajax({
				...
				success:function(resp){
					if(resp.status == 'success'){
						location.replace(_base_url_+"shop/?page=sales/view_sale&id="+resp.id);
